...
class Group:
    ...
    def handle(self, msg):
        ...
        for func in self.process_order:
            try:
                ...
                if self.__operator.sys_paras['debug']: print traceback.format_exc()
    ...
    def reply(self, reply_content, fail_times=0):
        ...
        rsp = ""
        fix_content = reply_content
        self.__operator.sendGroupMessage(self.gid, fix_content)
    ...
    def gamble_fqzs_end(self, face, result):
        ...
        for item in result:
            ...
            if user_nick: 
                ...
                if item['earning'] > 0:
                    reply_msg += "\n[喝彩]【{0}】赢得了 {1} 积 分".format(user_nick, self.format_long_number(item['earning']))
                elif item['earning'] < 0:
                    reply_msg += "\n[发脾气]【{0}】输掉了 {1} 积 分".format(user_nick, self.format_long_number(-item['earning']))
                else:
                    reply_msg += "\n[棒棒糖]【{0}】押得四平八稳，没赔也没赚。".format(user_nick)
        self.reply(reply_msg)
